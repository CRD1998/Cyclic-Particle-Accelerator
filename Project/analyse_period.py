...
"""
This file is an independent file that tests the simulated time period of a proton in a constant 
magnetic field against the expected time period given by:

T = 2*pi*m / (qB)

The simulation will produce a plot showing the proton in the magentic field. It will also produce
a pickled csv file containing the time periods of the five simulated revoltions. If you do not have
...
def simulation():
    ...
    data_csv = []
    revolution = 1
    data_csv.append([revolution,time,0,[0,0,0]]) # ensure data always has one element to avoid length error in next line
...
print('time period from theory: {0} s'.format(round(theoretical_period,15)))
print('raito of simulated time period to theoretical: {0}'.format(round((average_period/theoretical_period),15)))